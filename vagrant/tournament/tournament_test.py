...
def testCount():
    ...
    if c == '0':
        raise TypeError(
            "countPlayers() should return numeric zero, "
            "not string '0'.")
    if c != 0:
        raise ValueError(
...
def testRegisterCountDelete():
    ...
    c = countPlayers()
    if c != 4:
        raise ValueError("After registering four players, "
                         "countPlayers should be 4.")
    deletePlayers()
    c = countPlayers()
    if c != 0:
        raise ValueError("After deleting, countPlayers should "
                         "return zero.")
...
def testStandingsBeforeMatches():
    ...
    if len(standings) < 2:
        raise ValueError(
            "Players should appear in playerStandings "
